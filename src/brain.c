...
int 
compare_ext_moves (const void *ext_a, const void *ext_b)
{
...
//	return b->nodes - a->nodes;
	...
	return history_lookup (tree, b->move) - history_lookup (tree, a->move);
}
...
static int
root_search (TREE *tree, int depth, ext_move* moves, int num_moves,
             int alpha, int beta)
{
	int i = 0;
	chi_pos tmp_pos;
	...
	for (i = 0; i < num_moves; ++i) {
		...
		if (pv_seen) {
			if (!xboard)
				print_current_move (tree, &tmp_pos, move, i + 1, num_moves, 
				                    -alpha - 1, -alpha);

			score = -search(tree, depth - ONEPLY, 1, -alpha - 1, -alpha, 1);
			...
			if ((score > alpha) && (score < beta)) {
				...
				score = -search(tree, depth - ONEPLY, 1, -beta, -alpha, 1);
			}
		} else {
	    	if (!xboard)
				print_current_move(tree, &tmp_pos, move, i + 1, num_moves, 
     ...
     chi_epd_pos* epd;
{
	...
	*mv = moves[0];
	...
	/* How much time will we afford ourselves? */
	if (pondering) {
...
void
store_killer (TREE *tree, chi_move move, int depth, int ply)
{
